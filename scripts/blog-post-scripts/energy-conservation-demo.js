...
Waiting for animation...
    ...
    function calculatePhysics() {
        ...
        // Energy conservation: KE_A + U_A = KE_B + U_B
        // (1/2)m*v_A^2 + U_A = (1/2)m*v_B^2 + U_B
        // (1/2)m*v_A^2 - (1/2)m*v_B^2 = U_B - U_A = deltaU
        // (1/2)m*(v_A^2 - v_B^2) = deltaU
        // v_B^2 = v_A^2 - 2*deltaU/m
        
        const vB_correct_squared = v0 * v0 - 2 * deltaU / m_e;
        const vB_correct = vB_correct_squared > 0 ? Math.sqrt(vB_correct_squared) : 0;
        
        // Incorrect method: (v_B - v_A)^2 = -2*deltaU/m
        // This assumes ΔK = (1/2)m*(v_B - v_A)^2 which is WRONG
        // v_B - v_A = ±sqrt(-2*deltaU/m)
        // Taking the positive root (assuming electron speeds up):
        // v_B = v_A + sqrt(-2*deltaU/m)
        
        const term = -2 * deltaU / m_e;
        let vB_incorrect;
        if (term > 0) {
            vB_incorrect = v0 + Math.sqrt(term);
        } else {
            // If term is negative, the sqrt is imaginary
            // The incorrect method would give: v_B = v_A - sqrt(2*deltaU/m)
            vB_incorrect = v0 - Math.sqrt(-term);
        }
    ...
    function animate() {
        ...
        // Get current charge values from sliders
    ...
    function finishAnimation(physics) {
        ...
        vbCorrect.textContent = physics.vB_correct.toExponential(3);
        vbIncorrect.textContent = physics.vB_incorrect.toExponential(3);
        
        // Calculate percent difference
        const percentDiff = Math.abs(physics.vB_correct - physics.vB_incorrect) / physics.vB_correct * 100;
        
        // Display comparison
        comparisonText.innerHTML = `
            <strong style="color: #000;">Difference:</strong> <span style="color: #000;">${Math.abs(physics.vB_correct - physics.vB_incorrect).toExponential(3)} m/s
            (${percentDiff.toFixed(2)}% relative difference)</span><br><br>
            ${percentDiff > 1 ? '<strong style="color: #d32f2f;">⚠️ Significant difference detected!</strong>' : 
                                '<span style="color: #000;">Difference is small for these parameters.</span>'}
        `;
        
        // Display detailed calculations
        calcDetails.textContent = `
Parameters: Q = ${(physics.Q * 1e9).toFixed(1)} nC, q = ${(physics.q * 1e9).toFixed(1)} nC, l = ${physics.l.toFixed(3)} m, v₀ = ${physics.v0.toExponential(3)} m/s

Energy: U_A = ${physics.U_A.toExponential(3)} J, U_B = ${physics.U_B.toExponential(3)} J, ΔU = ${physics.deltaU.toExponential(3)} J

CORRECT: v²_B = v²_A - 2ΔU/m → v_B = ${physics.vB_correct.toExponential(4)} m/s

INCORRECT: v_B = v_A + √(-2ΔU/m) → v_B = ${physics.vB_incorrect.toExponential(4)} m/s

Difference: ${Math.abs(physics.vB_correct - physics.vB_incorrect).toExponential(3)} m/s (${(Math.abs(physics.vB_correct - physics.vB_incorrect) / physics.vB_correct * 100).toFixed(2)}%)
        `;
